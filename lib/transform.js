...
class Transform {
  constructor () {
    ...
    //  Not totally sure why that doesn't happen for
    //  free...
    this.inv = {
      is_inverse : true,
      ...
      orient_quat : self.unorient_quat.bind(self),
      orient_quat_ip : self.unorient_quat_ip.bind(self),
      unorient_quat : self.orient_quat.bind(self),
      unorient_quat_ip : self.orient_quat_ip.bind(self)
    };
  }
...
//   untransform_quat_ip ( quat ) {}
   ...
   * @return {UnitQuaternion} the untransformed orientation quaternion
   */
  unorient_quat ( quat ) { return quat; }
  /**
   * Transform an orientation quaternion in place
   *
   * Note, the provided quaternion is mutated by this method.
...
const EuclideanReverseTransformMixin = (Sub) => class extends Sub {
  constructor () {
    ...
    this.matrix = (() => this.euclidean.matrix )

    // Configure the inverse object, this is necessary for the
    // `network.CoordinateSystem.get_euclidean_transform_to` method
    // to both recognize `this.inv` as a Euclidean-type transform,
    // and to be able to composite this transform with other
    // Euclidean-type transforms to get the total effect ET
    this.inv.is_euclidean = true;
    this.inv.is_homogeneous = true;
    this.inv.matrix = (() => this.euclidean.matrix_inv )
    this.inv.euclidean = (() =>  this.euclidean.get_inv()) // HACK : generate the inverse euclidean object
  }
  ...
  orient_quat( quat ) { return this.euclidean.orient_quat( quat ); }
  unorient_quat ( quat ) { return this.euclidean.unorient_quat( quat ); }
  orient_quat_ip ( quat ) { this.euclidean.orient_quat_ip( quat ); }
  unorient_quat_ip ( quat ) { this.euclidean.unorient_quat_ip( quat ); }
}
...
class CompositeTransform extends CompositeTransformMixin(Transform) {
  ...
  orient_quat ( quat ) { return this.second.orient_quat(this.first.orient_quat( quat )); }
  unorient_quat ( quat ) { return this.second.unorient_quat(this.first.unorient_quat( quat )); }
  orient_quat_ip ( quat ) { this.first.orient_quat_ip( quat ); this.second.orient_quat_ip( quat ); }
  unorient_quat_ip ( quat ) { this.first.unorient_quat_ip( quat ); this.second.unorient_quat_ip( quat ); }
}
...
class PinholeCameraTransform extends Transform {
  ...
  constructor ( Q_key ) {
    ...
    this.is_homogeneous = true;
    this.matrix = (() => this.camera.Q_inv);
    this.inv.is_homogeneous = true;
    this.inv.matrix = (() => this.camera.Q);
  }
